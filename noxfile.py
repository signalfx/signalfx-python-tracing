...
def install_unit_tests(session, *other_packages):
    ...
    return session.install(
        "attrs==19.1.0", pytest, f"{sdist}[unit_tests]", *other_packages
    )
...
def flake8(session):
    ...
    pip_freeze(session)
    session.run(
        "flake8", "setup.py", "scripts", "signalfx_tracing", "tests", "noxfile.py"
    )
...
def unit(session):
    ...
    pip_freeze(session)
    session.run(
        "pytest", "tests/unit", "--ignore", "tests/unit/libraries", "-p", "no:django"
    )
...
@nox.parametrize(
    "celery",
    (">=3.1,<3.2", ">=4.0,<4.1", ">=4.2,<4.3", ">=4.3,<4.4", ">=4.4.0-rc1,<4.5"),
)
...
def django18_via_bootstrap(session):
    ...
    install_unit_tests(
        session, "django>=1.8,<1.9", "pytest-django<4.0", "django-opentracing"
    )
...
def test_elasticsearch(session, image_version):
    ...
    session.run(
        ...
        image_version,
        "tests/integration/elasticsearch_",
    )
...
@nox.session(python=("2.7", "3.5", "3.6", "3.7"), reuse_venv=True)
@nox.parametrize(
    "elasticsearch", (">=6.0,<6.1", ">=6.1,<6.2", ">=6.2,<6.3", ">=6.3,<6.4")
)
...
@nox.session(python=("2.7", "3.5", "3.6", "3.7"), reuse_venv=True)
@nox.parametrize(
    "tornado", (">=4.3,<4.4", ">=4.4,<4.5", ">=4.5,<5.0", ">=5.0,<5.1", ">=5.1,<5.2")
)
