...
from signalfx_tracing.libraries.tornado_.instrument import (
    config,
    instrument,
    uninstrument,
)
...
class TestTornadoConfig(TornadoTestSuite):
    @pytest.mark.parametrize(
        "trace_all, trace_client, traced_attributes, start_span_cb",
        [
            (True, True, ["attr_one", "attr_two"], None),
            (False, False, [], start_span_cb),
        ],
    )
    def test_instrument_tornado_values_set_by_config(
        self, trace_all, trace_client, traced_attributes, start_span_cb
    ):
        tracer = MockTracer()
        ...
        with mock.patch("tornado.web.Application", MockApplication):
            ...
            assert (
                app.settings.get("opentracing_traced_attributes") is traced_attributes
            )
